...
description: "Sandbox/testnet anchor integration with Etherfuse FX API for Kivo, including onboarding, assets, quotes, orders, webhooks, Stellar claim transactions, x402 funding, and deployment."
---

# Etherfuse Integration

This document specifies how Kivo integrates with the Etherfuse FX API as the fiat/stablecoin anchor layer for Stellar testnet demos and future production ramps.
...
Etherfuse does not turn hardware on and does not replace Kivo Gateway. The gateway releases access only after the Kivo payment engine validates payment state and issues an authorization. Etherfuse gives the user, operator, and demo reviewer a clearer funding/anchor story around the asset used in the flow.
